...
$randomlySelectedOptions = $user->selectRandomWordByLimit(10);
$optionsArr = array();
$optionsArr[] = $randomLySelectedWord['japanese'];
foreach ($randomlySelectedOptions as $option) {
    if(count($optionsArr) == 4){
        break;
    }
    if(!in_array($option['japanese'], $optionsArr)){
        $optionsArr[] = $option['japanese'];
    }
}
shuffle($optionsArr);
?>
...
<div class="my-body">
    ...
    <div class="col-md-offset-3 col-md-6 my-form">
        <form class="form-horizontal" method="POST" action="mcq_result.php">
            <h2 class="text-center exam-ques"><?php echo strtoupper($randomLySelectedWord['english'])?></h2>
            <div class="form-group">
                <label class="control-label col-sm-offset-2 col-sm-2" for="japanese">Japanese:</label>
                <div class="col-sm-8">
                    <?php 
                        foreach($optionsArr as $singleOption){
                            $single = json_decode($singleOption);
                            $res = '';
                            foreach ($single as $arr) {
                                $res .= chr($arr);
                            }
                            ?>
                            <input type="radio" name="japanese" value="<?php echo $res ?>"><?php echo $res ?>
                        <?php }
                    ?>
            ...
            <div class="form-group">
                <div class="col-sm-offset-4 col-sm-8">
                    ...
                    <a href="mcq_exam_english_to_japanese.php" class="btn btn-warning">Skip</a>
                </div>
            </div>
        </form>
    </div>
